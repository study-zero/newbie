...
- LoadTest 객체가 생성될 때, LoadTest 사용자 정의 컨트롤러는 어떻게 동작하는가?
  (1) 먼저 API 서버가 CRD 객체 생성을 감지하고 이벤트를 발생시킴
  (2) 사용자 정의 컨트롤러가 CRD 객체 생성 이벤트를 로직에 맞게 처리 (디플로이먼트를 생성 / 다른 리소스 생성 등)
...
- 오퍼레이터
    ...
    - 오퍼레이터 패턴을 따른다는 것은? 
      - ReplicaSet 이 항상 정의된 pod 의 개수를 유지하듯, 
      사용자 정의 리소스도 항상 정의된 상태를 유지하도록 컨트롤러 개발하는 것
